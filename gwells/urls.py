...
urlpatterns = [
    ...
    url(r'^' + app_root_slash + 'robots\.txt$', TemplateView.as_view(template_name='robots.txt',
    ...
    url(r'^' + app_root_slash + 'submission/(?P<pk>[0-9]+)$', ActivitySubmissionDetailView.as_view(),
...
if settings.ENABLE_DATA_ENTRY:
    urlpatterns = [
        url(r'^' + app_root_slash + 'submission/create$', ActivitySubmissionWizardView.as_view(views.FORMS),
            name='activity_submission_create'),
        url(r'^' + app_root_slash + 'submission/$', ActivitySubmissionListView.as_view(),
