    ...
    if ("DEV".equalsIgnoreCase(stageDeployName)){
        _stage("Load Fixtures - ${stageDeployName}", context) {
            node('master'){
                ...
                "oc exec '${podName}' -n '${projectName}' -- bash -c 'cd /opt/app-root/src && pwd && python manage.py flush --no-input'"
                ...
                sh "oc exec '${podName}' -n '${projectName}' -- bash -c 'cd /opt/app-root/src && pwd && gzip -d gwells/fixtures/wellsearch.json.gz'"
