    ...
    if ("DEV".equalsIgnoreCase(stageDeployName)){
        _stage("Load Fixtures - ${stageDeployName}", context) {
            node('master'){
                ...
                sh "oc exec '${podName}' -n '${projectName}' -- bash -c 'cd /opt/app-root/src && pwd && export LD_LIBRARY_PATH=/opt/rh/rh-python35/root/usr/lib64/ && python manage.py flush --no-input'"
