        ...
        stage('DEMO - API Tests') {
            ...
            steps {
                script {
                    _openshift(env.STAGE_NAME, TOOLS_PROJECT) {
                        podTemplate(
                            label: "nodejs-${APP_NAME}-${DEMO_SUFFIX}-${PR_NUM}",
                            name: "nodejs-${APP_NAME}-${DEMO_SUFFIX}-${PR_NUM}",
                            serviceAccount: 'jenkins',
                            cloud: 'openshift',
                            activeDeadlineSeconds: 1800,
                            containers: [
                            ...
                            node("nodejs-${APP_NAME}-${DEMO_SUFFIX}-${PR_NUM}") {
                                checkout scm
                                dir('api-tests') {
                                    sh 'npm install -g newman'
                                    String BASEURL = "https://${DEMO_HOST}/gwells"
                                    try {
                                        sh """
                                            newman run ./registries_api_tests.json \
                                                --global-var test_user=\$GWELLS_API_TEST_USER \
