...
class SurveyView(LoginRequiredMixin, View):
    ...
    fields = ['survey_introduction_text',
              'survey_link', 'survey_page', 'survey_enabled']
    ...
    def __create_or_update_survey(self, request, method, form_number=0, **kwargs):
        ...
        if method.upper() == 'PUT':
            ...
            survey = Survey.objects.get(
                pk=fields[method].get(form_fields['SURVEY_GUID']))
        else:
            return HttpResponseNotAllowed(methods)

        survey.survey_introduction_text = fields[method].get(
            form_fields['SURVEY_INTRODUCTION_TEXT'])
