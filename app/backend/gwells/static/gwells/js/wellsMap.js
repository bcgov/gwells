...
function WellsMap(options) {
  ...
  var _setInitialExtentBounds = function(initialExtent) {
    ...
    if (
      typeof initialExtent.startCorner === 'string' &&
      typeof initialExtent.endCorner === 'string'
    ) {
      var delimiter = ','
      var startLatLongArray = initialExtent.startCorner
        .split(delimiter)
        .map(function(val) {
          return parseFloat(val)
        })
      var endLatLongArray = initialExtent.endCorner
        .split(delimiter)
        .map(function(val) {
          return parseFloat(val)
        })
      if (startLatLongArray.length === 2 && endLatLongArray.length === 2) {
        var startLatLng = _getLatLngInBC(
          startLatLongArray[0],
          startLatLongArray[1]
        )
  ...
  var _drawWells = function(wells, fitBounds) {
    ...
    if (
      _exists(_wellPushpin) &&
      _exists(_wellPushpin.wellDetails) &&
      _exists(_wellPushpin.wellDetails.guid)
    ) {
  ...
  var _searchWellsInBoundingBox = function() {
    if (
      _exists(_leafletMap) &&
      _leafletMap.getZoom() >= _AJAX_SEARCH_MIN_ZOOM_LEVEL
    ) {
  ...
  var _createExternalQueryControl = function() {
    var container = L.DomUtil.create(
      'div',
      'leaflet-control leaflet-area-search'
    )
  ...
  var _placeExternalQueryControl = function() {
    if (
      _leafletMap.getZoom() < _EXTERNAL_QUERY_MIN_ZOOM_LEVEL &&
      _exists(_leafletMap.externalQueryControl)
    ) {
      _leafletMap.removeControl(_leafletMap.externalQueryControl)
    } else if (
      _leafletMap.getZoom() >= _EXTERNAL_QUERY_MIN_ZOOM_LEVEL &&
      !_exists(_leafletMap.hasExternalQueryControl)
    ) {
  ...
  var _createRectangleZoomControl = function() {
    var container = L.DomUtil.create(
      'div',
      'leaflet-control leaflet-select-zoom'
    )
    return L.Control.extend({
      onAdd: function(map) {
        L.DomEvent.on(
        ...
        container.title =
          'Click or tap, then drag area of interest on the map.  Map will zoom to that location.'
  ...
  var _createGeolocationControl = function() {
    var container = L.DomUtil.create(
      'div',
      'leaflet-control leaflet-geolocation'
    )
    return L.Control.extend({
      onAdd: function(map) {
        L.DomEvent.on(
  ...
  var placeWellPushpin = function(latLongArray, wellDetails) {
    ...
    if (_exists(_wellPushpin) && _exists(_wellPushpin.pushpinMarker)) {
      ...
      _wellPushpin.wellMarker = L.circleMarker(
        latLong,
        _WELL_PUSHPIN_WELL_MARKER_STYLE
      ).addTo(_leafletMap)
  ...
  ;(function(options) {
    ...
    // Bounds
    var initialExtentBounds = _exists(options.initialExtent)
      ? _setInitialExtentBounds(options.initialExtent)
      : void 0
    _maxBounds = _exists(options.mapBounds)
      ? _setMaxBounds(options.mapBounds)
      : void 0
    ...
    if (
      _exists(wellPushpinInit) &&
      _exists(wellPushpinInit.lat) &&
      _exists(wellPushpinInit.long)
    ) {
    ...
    if (_exists(options.externalAttributionNodeId)) {
      $('#' + options.externalAttributionNodeId).append(
        _leafletMap.attributionControl.getContainer()
      )
    }
