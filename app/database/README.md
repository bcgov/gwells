...
- Install DBeaver
- Install OC terminal
- Login to Openshift on your terminal
- Have have access to the Openshift project/the right environment
- Find name of DB pod:
  - Go into the Openshift console
  - Select the project/the right environment (Project name in the form of "26e83e-prod" or similar)
  - In Developer mode, go the the `Topology` tab
  - Find the pod with the Postgres symbol (Postgres Elephant)
- Port forward into the pod:
  - In a terminal, type `oc port-forward <pod name> 5432:5432`
- Find DB login secrets:
  - In Openshift console, go to the `Secrets` tab
  - Locate in the list `gwells-database-secrets`
- Create a connection in DBeaver to the DB pod
  - Go to DBeaver, top menu go "Database" -> "Create a new connection"
  - Fill in the following values in the connection configuration
  - Host: `localhost` / Port: `5432`
  - Database: `gwells` (or "database-name" in DB login secrets)
  - Authentication: select `Database Native`
  - Username: "database-user" in DB login secrets
  - Password: "database-password" in DB login secrets
