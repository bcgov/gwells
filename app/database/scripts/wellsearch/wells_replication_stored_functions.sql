...
DECLARE
  ...
  SELECT
    ...
    owner.phone_number AS owner_tel                                          ,
    ...
    wells.utm_north                                    ,
    wells.utm_east                                     ,
    wells.utm_accuracy_code AS coordinate_acquisition_code,
    wells.bcgs_id                                      ,
...
BEGIN
  ...
  CREATE unlogged TABLE IF NOT EXISTS xform_well (
     ...
     owner_tel                          character varying(25),
     owner_email                        character varying(80),
     ...
     utm_easting                        integer,
     coordinate_acquisition_code         character varying(10),
     bcgs_id                            bigint,
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(xform.owner_tel, ' ')               ,
    COALESCE(xform.owner_email, ' ')              ,
    ...
    CASE
      WHEN xform.latitude is NULL OR
        ...
        NULL
      ELSE
          ST_SetSRID(ST_MakePoint(longitude, latitude),4326) -- Values are BC Albers. but we are using WGS84 Lat Lon to avoid rounding errors
    END AS geom                             ,
