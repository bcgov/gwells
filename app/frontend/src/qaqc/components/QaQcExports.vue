    ...
    exportBaseUrl () {
      ...
      switch (this.selectedTab) {
        case 0: // record compliance
          return `${process.env.AXIOS_BASE_URL}qaqc/recordcompliance/download`
        case 1: // mislocated wells
          return `${process.env.AXIOS_BASE_URL}qaqc/mislocatedwells/download`
        case 2: // cross referencing
          return `${process.env.AXIOS_BASE_URL}qaqc/crossreferencing/download`
        default:
          return `${process.env.AXIOS_BASE_URL}wells/export` // Default or fallback URL
      }
    }
  },
  methods: {
