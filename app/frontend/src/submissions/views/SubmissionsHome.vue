  ...
  methods: {
    ...
    formSubmit() {
      ...
      if (this.isStaffEdit) {
        ...
        let skipLatLon =
          "latitude" in meta.valueChanged || "longitude" in meta.valueChanged;
        let skipWorkDates =
          "work_start_date" in meta.valueChanged ||
          "work_end_date" in meta.valueChanged;
        let skipConDates =
          "construction_start_date" in meta.valueChanged ||
          "construction_end_date" in meta.valueChanged;
        let skipAltDates =
          "alteration_start_date" in meta.valueChanged ||
          "alteration_end_date" in meta.valueChanged;
        let skipDecDates =
          "decommission_start_date" in meta.valueChanged ||
          "decommission_end_date" in meta.valueChanged;
        let skipGroundElevation =
          "ground_elevation" in meta.valueChanged ||
          "ground_elevation_method" in meta.valueChanged;
        let doNotSkip = [
        ...
        Object.keys(data).forEach((key) => {
          ...
          if ((key === "latitude" || key === "longitude") && skipLatLon) {
            return;
          }
          if (
            (key === "ground_elevation" || key === "ground_elevation_method") &&
            skipGroundElevation
          ) {
            return;
          }
          ...
          if (
            (key === "alteration_start_date" ||
              key === "alteration_end_date") &&
            skipAltDates
          ) {
            if (data[key] === "") {
              data[key] = null;
          ...
          if (
            (key === "decommission_start_date" ||
              key === "decommission_end_date") &&
            skipDecDates
          ) {
            if (data[key] === "") {
              data[key] = null;
          ...
          if (
            key !== "well" &&
            !(key in meta.valueChanged) &&
            !doNotSkip.includes(key)
          ) {
            delete data[key];
          }
        });
      ...
      if (
        data.company_of_person_responsible &&
        data.company_of_person_responsible.org_guid
      ) {
        data.company_of_person_responsible =
          data.company_of_person_responsible.org_guid;
      }
      ...
      data.aquifer_parameters_set = data.aquifer_parameters_set.map(
        (item, index) => {
      ...
      const savingNotification = this.$noty.info(
        '<div class="loader"></div><div class="notifyText">Saving...</div>',
        { timeout: false }
      );
      ...
      ApiService.post(PATH, data)
        .then((response) => {
          ...
          if (this.isStaffEdit) {
            this.$nextTick(() => {
              this.$noty.success(
                '<div class="notifyText">Changes saved.</div>',
                { killer: true }
              );
            });
            ...
            this.$nextTick(() => {
              this.$noty.success(
                '<div aria-label="Close" class="closeBtn">x</div><div class="notifyText">Well report submitted.</div>',
                { killer: true }
              );
            });
    ...
    showErrorMessages() {
      ...
      // Error notifications
      this.$noty.error('<div class="errorTitle">' + errTxt + "</div>", {
        timeout: 2000,
        killer: true,
      });
      cleanErrors.forEach((e) => {
        this.$noty.error(
          '<div aria-label="Close" class="closeBtn">x</div><div class="errorText"><b>Error: </b>' +
            e +
            "</div>",
          { timeout: false }
        );
    ...
    validateWellIdentificationPlateFields(errors) {
      ...
      const validateWellClasses = [
        WELL_CLASS.WATER_SUPPLY,
        WELL_CLASS.INJECTION,
        WELL_CLASS.RECHARGE,
      ];
      const isWellIdentificationPlateToBeVerified =
        validateWellClasses.includes(well_class);

      if (isWellIdentificationPlateToBeVerified == false) {
        return;
      }

      if (!identification_plate_number) {
        errors.identification_plate_number = [
          "Identification Plate Number Required.",
        ];
      }
      if (!well_identification_plate_attached) {
        errors.well_identification_plate_attached = [
          "Where Identification Plate Attached Required.",
        ];
      }
    },
    ...
    newlyConstructedWellValidation(errors) {
      const { work_start_date, work_end_date } = this.form;

      const mandatoryLicensingDate = NEW_WELL_CONSTRUCTION_VALIDATION_DATE;

      const workStartDatePastWorkEndDate =
        work_start_date !== "" &&
        work_end_date !== "" &&
        work_start_date > work_end_date;
      const workEndDatePastMandatoryLicensingDate =
        work_end_date !== "" && work_end_date >= mandatoryLicensingDate;
      const workStartDatePastMandatoryLicensingDate =
        work_start_date !== "" && work_start_date >= mandatoryLicensingDate;

      if (this.activityType !== TYPE_OF_WORK.CON) {
        return;
      }
      ...
      if (
        workStartDatePastMandatoryLicensingDate ||
        workEndDatePastMandatoryLicensingDate
      ) {
    ...
    isWellLocationFilled(errors) {
      ...
      const legalDescriptionValidate = legalDescriptionFields.some(
        (item) => !!item
      );
      ...
      if (
        locationAddressValidate ||
        legalDescriptionValidate ||
        pidAddressValidate
      ) {
        return;
      }
    ...
    isFormValid() {
      ...
      if (
        (this.activityType === "ALT" || this.activityType === "DEC") &&
        this.form.well
      ) {
      ...
      if (
        this.$refs.activitySubmissionForm &&
        this.$refs.activitySubmissionForm.$refs.wellCoords
      ) {
        const wellCoordsNotWithinBC =
          !this.$refs.activitySubmissionForm.$refs.wellCoords.validCoordinate;
      ...
      if (validateWellClassAndIntendedWaterUse) {
        ...
        if (!this.form.well_class) {
          ...
          if (
            !this.form.intended_water_use ||
            this.form.intended_water_use === "NA"
          ) {
            errors.intended_water_use = [
              "Intended water use is required when the well class is Water Supply.",
            ];
          }
        } else {
          // all other well_class_codes
          if (
            this.form.intended_water_use &&
            this.form.intended_water_use !== "NA"
          ) {
            errors.intended_water_use = [
              "Intended water use only valid for a well class of Water Supply.",
            ];
          }
        }
      }
      ...
      if (
        !this.form.artesian_conditions &&
        (this.form.artesian_flow > 0 ||
          this.form.artesian_pressure > 0 ||
          this.form.artesian_pressure_head > 0)
      ) {
        errors.artesian_conditions = [
          "Set Artesian Well to Yes for positive flow or pressure.",
        ];
      }
    ...
    handlePreviewButton() {
      if (!this.formChanges()) {
        this.$noty.info(
          '<div class="errorTitle">Please add some data to your submission.</div>',
          { killer: true }
        );
        return;
      }
    ...
    fetchFiles() {
      ...
      let tag =
        this.form.well && isNaN(this.form.well)
          ? this.form.well.well_tag_number
          : this.form.well;

      if (tag) {
        ApiService.query(`wells/${tag}/files`).then((response) => {
    ...
    fetchWellDataForStaffEdit(options = {}) {
      ...
      ApiService.query(`wells/${this.$route.params.id}/edit`)
        .then((res) => {
          ...
          if (
            ...
            this.form.person_responsible.name === this.form.driller_name
          ) {
