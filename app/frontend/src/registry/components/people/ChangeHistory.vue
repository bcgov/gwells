      ...
      <div id="historyList" ref="history">
        ...
        <div class="mt-2" v-if="history && history.length">
          <div class="mt-3" v-for="(version, index) in history" :key="`history-version ${index}`" :id="`history-version-${index}`">
              <span class="font-weight-bold">{{ version.user }}</span> ({{ version.date | moment("MMMM Do YYYY [at] LT") }}):
              <div class="ml-4">
                <div v-if="version.created">Created record</div>
                ...
                <div
                  ...
                  {{ key }} changed from {{ version.prev[key] }}<span v-if="!version.prev[key]">(blank)</span> to {{ value }}<span v-if="!value">(blank)</span>
                </div>
              </div>
          </div>
        </div>
...
export default {
   ...
   * resource: the name of the API/DB resource that the record belongs to e.g. "organizations"
