<template>
  <div>
    <template v-for="(reg, regIndex) in driller.registrations">
      <div v-if="reg.activity === 'DRILL' && reg.applications && reg.applications.length" :key="`reg quals ${driller.person_guid} ${regIndex}`">
        <div v-for="(app, appIndex) in reg.applications" :key="`app quals ${driller.person_guid} ${app.application_guid} ${appIndex}`">
