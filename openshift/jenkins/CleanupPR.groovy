...
if (ghEventType == 'pull_request' && payload['action'] == 'closed' && prNum) {
    ...
    deleteCreatedObjects.waitForOrKill(5000)
    println "out> $sout err> $serr"

    // these objects were generated by openshift for PVC provisioning
    sout = new StringBuilder()
    serr = new StringBuilder()
    def deleteGeneratedObjects = "oc delete all,pvc,secret,configmap -n ${DEV_PROJECT} -l gluster.kubernetes.io/provisioned-for-pvc=gwells-pgsql-dev-pr-${prNum}".execute()
    deleteGeneratedObjects.consumeProcessOutput(sout, serr)
    deleteGeneratedObjects.waitForOrKill(5000)
    println "out> $sout err> $serr"
